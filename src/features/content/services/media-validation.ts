...
export async function validateMediaFiles(
  ...
  const probeAudioDuration =
    options.probeAudioDuration ??
    (await import("@/features/content/services/media-validation-audio-probe")).defaultAudioDurationProbe;
  ...
  if (files.audio) {
    ...
    const acceptedAudioMimeMap = new Map<string, string[]>([
      ["audio/mpeg", ["audio/mpeg"]],
      ["audio/wav", ["audio/wav"]],
      ["audio/mp4", ["audio/mp4", "audio/x-m4a"]],
    ]);

    const acceptedFileMimeTypes = signature ? acceptedAudioMimeMap.get(signature.mimeType) : null;

    if (
      !signature ||
      signature.kind !== "audio" ||
      !acceptedFileMimeTypes ||
      !acceptedFileMimeTypes.includes(files.audio.type)
    ) {
      reject("Arquivo recusado. Verifique formato, tamanho e quantidade antes de tentar novamente.", "formato");
    }
